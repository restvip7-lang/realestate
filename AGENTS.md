...
- Тексты интерфейса — в `src/i18n/messages/{ru,en,tr}.json`. Основной словарь — `ru.json`: если ключа нет в `en`/`tr`, показывается русский текст (`src/i18n/request.ts`). Короткие подписи переводим сразу, длинные тексты страниц — по мере готовности переводов.
