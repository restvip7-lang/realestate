...
- `/sitemap.xml`, `/robots.txt`, `/llms.txt` строятся сами из базы (раз в час). Пока нет `SITE_INDEXABLE=1`, robots.txt закрывает весь сайт; после добавления переменной нужен Redeploy (robots и метатеги собираются при сборке). После включения добавить сайт и `sitemap.xml` в Google Search Console и Яндекс Вебмастер.
